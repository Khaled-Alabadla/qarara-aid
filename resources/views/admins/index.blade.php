...
@section('title', 'عرض قائمة المسؤولين')
                            ...
                            <tbody>
                                @forelse ($admins as $admin)
                                    <tr>
                                        ...
                                        <td>{{ $admin->role == 'admin' ? 'مسؤول' : 'مشرف أعلى' }}</td>
                                        <td>{{ $admin->family_size }}</td>
                                        <td>{{ $admin->phone }}</td>
                                        <td>
                                            <a href="{{ route('admins.edit', $admin->id) }}"
                                                class="btn btn-primary-gradient btn-sm"> تعديل</i></a>
                                            <a class="btn btn-danger-gradient btn-sm" data-target="#modaldemo1" data-toggle="modal"
                                                href="" data-id="{{ $admin->id }}">حذف</a>

                                        </td>
                                    </tr>
                                @empty
                                    <td class="text-center" colspan="6">لا يوجد أي مسؤول</td>
                                @endforelse ($admins as $admin)
    ...
    <script>
      ...
      <script>
        $('#modaldemo1').on('show.bs.modal', function (event) {
            ...
            var modal = $(this);
            modal.find('form').attr('action', '/admins/' + adminId);
        });
    </script>
@endsection
